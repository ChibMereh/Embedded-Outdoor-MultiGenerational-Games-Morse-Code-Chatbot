...
class MorseCodeChatbot:
    ...
    def __init__(self):
        ...
        self.blehandler = None                 # will hold the active BLE handler once setupble() runs
    ...
    def sendresponse(self, responsetext):
        ...
        The text is written directly to responseChar so the Arduino can display
        it on its LCD.
        ...
        if hasattr(self.blehandler, "sendresponse"):    # check the BLE handler has a send method
            if ENABLEDISPLAY:
            ...
            self.blehandler.sendresponse(responsetext) # write the text to the Arduino's responseChar
        else:
    ...
    def setupble(self):
        ...
        try:
            ...
            self.blehandler = BLECentralHandler(
            ...
            logger.info("BLE central handler created")              # log that setup is done
        except Exception as e:
            logger.error(f"Failed to setup BLE handler: {e}")    # log the error
            raise                                                       # re-raise so the caller can handle it
    
    def run(self):
        """Run the main application loop"""
        try:
            self.setupble()            # create and configure the BLE handler
            
            if ENABLEDISPLAY:
                print("=" * 60)         # print a top separator
                print("Morse Code Decoder - Chatbot")   # print the application title
                print("Input method: BLE")
            ...
            # Start the BLE handler – this begins scanning/listening
            self.blehandler.start()
            try:
                while self.blehandler.running:   # keep the main thread alive while the handler is running
    ...
    def cleanup(self):
        """Clean up resources"""
        if self.blehandler:
            self.blehandler.cleanup()    # tell the BLE handler to close its connections and threads
